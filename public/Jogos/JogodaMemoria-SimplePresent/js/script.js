(function(){
    ...
    for(var i = 0; i < 16; i++){
        var img = {
            src: "/Jogos/JogodaMemoria-SimplePresent/img/" + i + ".jpg",
            id: i % 8
        };
        images.push(img);
    }
    ...
    function ramdomSort(oldArrey){

        var newArrey = [];

        while(newArrey.length !== oldArrey.length){
            var i = Math.floor(Math.random() * oldArrey.length);

            if(newArrey.indexOf(oldArrey[i]) < 0){
                newArrey.push(oldArrey[i]);
            }
        }
